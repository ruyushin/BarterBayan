...
const DESCRIPTION_MAX = 300;        // ← raised from 160
const ADDITIONAL_DESC_MAX = 300;    // ← new constant
...
// Adjust the required fields to match your Firestore user document schema
type ProfileStatus = "loading" | "complete" | "incomplete";
...
export default function AddItemScreen() {
  ...
  useEffect(() => {
    ...
    if (!uid) { setProfileStatus("incomplete"); return; }
    checkProfileComplete(uid)
      .then((ok) => setProfileStatus(ok ? "complete" : "incomplete"))
  ...
  const pickMedia = async () => {
    if (isAtLimit) { Alert.alert("Limit reached", "You can only add up to 10 files."); return; }

    const permission = await ImagePicker.requestMediaLibraryPermissionsAsync();
    if (!permission.granted) { Alert.alert("Permission needed", "Please allow access to your media."); return; }
    ...
    if (!result.canceled && result.assets && result.assets.length > 0) {
      ...
      for (const asset of result.assets) {
        ...
        if (originalUriSetRef.current.has(dedupKey)) { duplicateCount++; continue; }

        const sizeCheck = await checkFileSizeValid(rawUri, isVideo);
        if (!sizeCheck.valid) { oversizedCount++; continue; }

        const safeUri = await toSafeUri(rawUri);
        originalUriSetRef.current.add(dedupKey);
        newMedia.push({ uri: safeUri, type: isVideo ? "video" : "image", originalUri: rawUri, dedupKey });
      }

      const warnings: string[] = [];
      if (oversizedCount > 0) warnings.push(`${oversizedCount} file(s) skipped — too large.\n• Images: max ${MAX_FILE_SIZE_MB}MB\n• Videos: max ${MAX_VIDEO_SIZE_MB}MB`);
      if (duplicateCount > 0) warnings.push(`${duplicateCount} duplicate file(s) were not added.`);
      if (warnings.length > 0) Alert.alert("Some files skipped", warnings.join("\n\n"));
  ...
  const handleSubmit = async () => {
    if (profileStatus !== "complete") {
      Alert.alert("Profile Incomplete", "Please complete your profile setup before listing an item.");
      return;
    }
    if (!title.trim()) { Alert.alert("Missing field", "Please enter an item title."); return; }
    if (!description.trim()) { Alert.alert("Missing field", "Please enter a description."); return; }
    if (media.length === 0) { Alert.alert("Missing field", "Please add at least one photo or video."); return; }
    if (!selectedCategory) { Alert.alert("Missing field", "Please select a category."); return; }
    if (!selectedCondition) { Alert.alert("Missing field", "Please select a condition."); return; }

    const parsedWeight = parseFloat(weightValue);
    if (!weightValue.trim() || isNaN(parsedWeight) || parsedWeight <= 0) {
      Alert.alert("Missing field", "Please enter a valid estimated weight."); return;
    }

    const parsedQty = parseInt(quantity, 10);
    if (!quantity.trim() || isNaN(parsedQty) || parsedQty < 1) {
      Alert.alert("Missing field", "Please enter a valid quantity (at least 1)."); return;
    }
    ...
    try {
      ...
      if (!currentUser) { Alert.alert("Error", "You must be logged in to add items."); setIsLoading(false); return; }
      ...
      await addItem({
        ...
        quantity: parsedQty,
        ...(additionalDescription.trim() ? { additionalDescription: additionalDescription.trim() } : {}),
      });
  ...
  // Dismiss keyboard before opening preview so it can't bleed into the modal
  ...
  const renderProfileBanner = () => {
    if (profileStatus === "loading" || profileStatus === "complete") return null;

    return (
      <Animated.View style={[styles.profileBanner, { transform: [{ scale: pulseAnim }] }]}>
        {/* left accent stripe */}
        <View style={styles.profileBannerStripe} />

        <View style={styles.profileBannerBody}>
          <View style={styles.profileBannerTop}>
            <View style={styles.profileBannerIconWrap}>
          ...
          <View style={styles.profileChecklist}>
            {[
              ...
              <View key={item.label} style={styles.profileCheckItem}>
                ...
                <Text style={styles.profileCheckText}>{item.label}</Text>
              </View>
            ))}
          </View>
          ...
          >
            ...
            <Text style={styles.profileBannerBtnText}>Set Up Profile</Text>
          </TouchableOpacity>
        </View>
      </Animated.View>
    );
  ...
  // ─── Render ───────────────────────────────────────────────────────────
  const formDisabled = profileStatus !== "complete";

  return (
    <SafeAreaView style={styles.container}>
      {/*
        KeyboardAvoidingView wraps ONLY the scroll content.
        Modals are siblings outside it so the keyboard can never affect them.
      */}
      <KeyboardAvoidingView
        style={{ flex: 1 }}
      ...
      >
        ...
        >
          ...
          {/* ── Dim overlay hint when profile incomplete ── */}
          {formDisabled && profileStatus !== "loading" && (
            <View style={styles.formDisabledHint}>
              <Ionicons name="lock-closed-outline" size={13} color="#aaa" />
              <Text style={styles.formDisabledHintText}>
                Complete your profile to unlock the form below
              </Text>
            </View>
          )}

          {/* Form fields – visually dimmed when profile is incomplete */}
          <View style={formDisabled ? styles.formDimmed : undefined} pointerEvents={formDisabled ? "none" : "auto"}>
              ...
              <View style={styles.limitProgressOuter}>
                <View
                  style={[
                    ...
                    { width: `${(media.length / 10) * 100}%` as any, backgroundColor: isAtLimit ? "#ff4444" : NAVY },
                  ]}
                />
              </View>
            </View>
            ...
            <TouchableOpacity
              ...
              onPress={() => { setIsCategoryOpen(!isCategoryOpen); setIsConditionOpen(false); setIsWeightUnitOpen(false); }}
            >
              ...
              <View style={styles.dropdownMenu}>
                {CATEGORIES.map((cat) => (
                  <TouchableOpacity
                    ...
                    onPress={() => { setSelectedCategory(cat.id); setIsCategoryOpen(false); }}
                  >
                    <Text style={[styles.dropdownItemText, selectedCategory === cat.id && styles.dropdownItemSelected]}>
                      {cat.label}
                    </Text>
                    {selectedCategory === cat.id && <Ionicons name="checkmark" size={16} color={NAVY} />}
                  </TouchableOpacity>
                ))}
              </View>
            )}
            ...
            <TouchableOpacity
              style={styles.dropdownButton}
              onPress={() => { setIsConditionOpen(!isConditionOpen); setIsCategoryOpen(false); setIsWeightUnitOpen(false); }}
            >
              <Text style={styles.dropdownText}>
                {CONDITIONS.find((c) => c.id === selectedCondition)?.label || "Select Condition"}
              </Text>
              ...
              <View style={styles.dropdownMenu}>
                {CONDITIONS.map((cond) => (
                  <TouchableOpacity
                    key={cond.id}
                    style={styles.dropdownItem}
                    onPress={() => { setSelectedCondition(cond.id); setIsConditionOpen(false); }}
                  >
                    <Text style={[styles.dropdownItemText, selectedCondition === cond.id && styles.dropdownItemSelected]}>
                      {cond.label}
                    </Text>
                    {selectedCondition === cond.id && <Ionicons name="checkmark" size={16} color={NAVY} />}
                  </TouchableOpacity>
                ))}
              </View>
            )}
            ...
            <View style={styles.weightRow}>
              ...
              <TouchableOpacity
                ...
                onPress={() => { setIsWeightUnitOpen(!isWeightUnitOpen); setIsCategoryOpen(false); setIsConditionOpen(false); }}
              >
                <Text style={styles.unitButtonText}>{weightUnit}</Text>
                <Ionicons name={isWeightUnitOpen ? "chevron-up" : "chevron-down"} size={16} color={NAVY} />
              </TouchableOpacity>
            </View>
            {isWeightUnitOpen && (
              <View style={[styles.dropdownMenu, { marginTop: -4 }]}>
                {WEIGHT_UNITS.map((unit) => (
                  <TouchableOpacity
                    key={unit}
                    style={styles.dropdownItem}
                    onPress={() => { setWeightUnit(unit); setIsWeightUnitOpen(false); }}
                  >
                    <Text style={[styles.dropdownItemText, weightUnit === unit && styles.dropdownItemSelected]}>
                      {unit}
                    </Text>
                    {weightUnit === unit && <Ionicons name="checkmark" size={16} color={NAVY} />}
                  </TouchableOpacity>
                ))}
              </View>
            )}
            ...
            <View style={styles.quantityRow}>
              <TouchableOpacity
                style={[styles.qtyButton, parseInt(quantity || "0", 10) <= 1 && styles.qtyButtonDisabled]}
                onPress={decrementQuantity}
                disabled={parseInt(quantity || "0", 10) <= 1}
              >
                <Ionicons name="remove" size={20} color={parseInt(quantity || "0", 10) <= 1 ? "#ccc" : NAVY} />
              </TouchableOpacity>
              <TextInput
            ...
            <View style={styles.labelRow}>
              ...
              <Text style={[styles.charCount, description.length >= DESCRIPTION_MAX && styles.charCountLimit]}>
            ...
            <View style={styles.labelRow}>
              ...
              <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                <Text style={[styles.charCount, additionalDescription.length >= ADDITIONAL_DESC_MAX && styles.charCountLimit]}>
                  {additionalDescription.length}/{ADDITIONAL_DESC_MAX}
                </Text>
            ...
            <TextInput
              ...
              onChangeText={(t) => setAdditionalDescription(t.slice(0, ADDITIONAL_DESC_MAX))}
              maxLength={ADDITIONAL_DESC_MAX}
            />
            ...
            <TouchableOpacity
              style={[
                styles.submitButton,
                (isLoading || formDisabled) && styles.submitButtonDisabled,
              ]}
            ...
            >
              {isLoading ? (
                ...
                <>
                  <Ionicons
                    name={formDisabled ? "lock-closed" : "cloud-upload"}
                    size={18}
                    color="white"
                  />
                  <Text style={styles.submitText}>
                    {formDisabled ? "Profile Required" : "Submit"}
                  </Text>
                </>
              )}
            </TouchableOpacity>
      ...
      {/*
        ── Preview Modal ──
        Intentionally placed OUTSIDE KeyboardAvoidingView so the keyboard
        can never shift or shrink the modal layout.
      */}
      <Modal
      ...
      {/*
        ── Success Modal ──
        Also outside KeyboardAvoidingView for the same reason.
      */}
      <Modal visible={successModalVisible} transparent animationType="none" statusBarTranslucent>
        <View style={styles.successOverlay}>
          <Animated.View
            style={[styles.successCard, { transform: [{ scale: scaleAnim }], opacity: opacityAnim }]}
          >
            ...
            <Animated.View
              style={[
                styles.checkCircle,
                { transform: [{ scale: checkAnim.interpolate({ inputRange: [0, 1], outputRange: [0.4, 1] }) }], opacity: checkAnim },
              ]}
            >
...
const styles = StyleSheet.create({
  ...
  sectionSub: { fontSize: 12, color: "#888", marginBottom: 8, marginHorizontal: 16 } as TextStyle,
  ...
  dropdownItem: { paddingVertical: 12, paddingHorizontal: 16, borderBottomWidth: 1, borderBottomColor: "#f0f0f5", flexDirection: "row", alignItems: "center", justifyContent: "space-between" } as ViewStyle,
  dropdownItemText: { fontSize: 14, color: "#333" } as TextStyle,
  dropdownItemSelected: { color: NAVY, fontWeight: "600" } as TextStyle,

  weightRow: { flexDirection: "row", alignItems: "center", marginHorizontal: 16, marginBottom: 8, gap: 8 } as ViewStyle,
  weightInput: { flex: 1, backgroundColor: "#f0f0f5", borderRadius: 12, padding: 12, fontSize: 14, color: "#111" } as TextStyle,
  unitButton: { flexDirection: "row", alignItems: "center", backgroundColor: "#f0f0f5", borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, gap: 6, minWidth: 72 } as ViewStyle,
  unitButtonText: { fontSize: 14, fontWeight: "600", color: NAVY } as TextStyle,

  quantityRow: { flexDirection: "row", alignItems: "center", marginHorizontal: 16, marginBottom: 12 } as ViewStyle,
  qtyButton: { width: 46, height: 46, backgroundColor: "#f0f0f5", borderRadius: 12, justifyContent: "center", alignItems: "center", flexShrink: 0 } as ViewStyle,
  qtyButtonDisabled: { opacity: 0.4 } as ViewStyle,
  quantityInput: { flex: 1, backgroundColor: "#f0f0f5", marginHorizontal: 6, height: 46, borderRadius: 12, fontSize: 16, fontWeight: "600", color: "#111", textAlign: "center", paddingHorizontal: 0 } as TextStyle,

  descriptionInput: { backgroundColor: "#f0f0f5", borderRadius: 12, padding: 12, height: 110, textAlignVertical: "top", fontSize: 14, color: "#111", marginBottom: 8, marginHorizontal: 16 } as TextStyle,

  submitButton: { backgroundColor: NAVY, flexDirection: "row", alignItems: "center", justifyContent: "center", paddingVertical: 14, borderRadius: 10, marginBottom: 30, marginTop: 16, marginHorizontal: 16 } as ViewStyle,
  ...
  modalVideo: { width: "100%", aspectRatio: 16 / 9, backgroundColor: "#000" } as any,
  ...
  successSubRow: { flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 24 } as ViewStyle,

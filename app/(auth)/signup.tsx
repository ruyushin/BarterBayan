...
export default function SignUpScreen() {
  ...
  const handleFacebookSignUp = async (accessToken: string) => {
    ...
    try {
      ...
      if (!data.email) {
        Alert.alert('Email Required', 'Unable to retrieve email from Facebook account.');
        setIsSubmitting(false);
        return;
      }
  ...
  const createEmailAccount = async (email: string, password: string) => {
    ...
    // Reset form and navigate to verification
    setEmail('');
    setPassword('');
    setConfirm('');
    setErrors({});
      ...
      <View style={styles.content}>
        ...
        <View style={[styles.inputContainer, { backgroundColor: inputBgColor }]}> 
          ...
          <TextInput
            ...
            placeholderTextColor="#999999"
            value={confirm}
            onChangeText={(text) => { setConfirm(text); setErrors((e) => ({ ...e, confirm: undefined })); }}
            secureTextEntry={!showPassword}
          />
        </View>

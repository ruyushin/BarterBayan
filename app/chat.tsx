...
function AvatarWithFallback({
    ...
    if (uri && !failed) {
        return (
            <Image source={{ uri }} style={[baseStyle, style]} onError={() => setFailed(true)} />
        );
    }
    return (
...
// ─── Message Context Menu Modal (FIX: white theme, hold-then-release trigger) ─
    ...
    return (
        <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
            <Pressable style={ctx.overlay} onPress={onClose}>
                {/* FIX: white popup with shadow that stands out from chat background */}
                <Pressable style={ctx.panel}>
                    {/* Emoji row — FIX: only one emoji per user */}
...
// FIX: White popup with drop shadow to stand out from light chat background
...
const eh = StyleSheet.create({
    ...
    closeBtn: {
        marginTop: 14, backgroundColor: "#f2f2f7", borderRadius: 12,
...
const editModal = StyleSheet.create({
    ...
    cancelBtn: {
        flex: 1, backgroundColor: "#f2f2f7", borderRadius: 10,
...
function ReplyBanner({
    message, ownerName, onCancel,
}: {
    message: any; ownerName: string; onCancel: () => void;
}) {
...
const rb = StyleSheet.create({
    wrap: {
        ...
        backgroundColor: "#f0f0f5", paddingHorizontal: 14, paddingVertical: 8,
        borderTopWidth: 1, borderTopColor: "#e0e0e0", gap: 10,
    },
...
const reactionsModal = StyleSheet.create({
    ...
    panel: {
        ...
        width: "80%", maxHeight: "60%", paddingHorizontal: 16, paddingTop: 16, paddingBottom: 12,
    },
...
// ─── Swipeable Message Row (FIX: bidirectional swipe based on message direction) ────
function SwipeableMessage({
    children, onSwipeReply, isMe,
}: {
    children: React.ReactNode; onSwipeReply: () => void; isMe: boolean;
}) {
    const translateX = useRef(new Animated.Value(0)).current;
    const triggered  = useRef(false);
    const THRESHOLD  = 60;

    // FIX: For user messages (isMe=true): swipe right-to-left (negative dx)
    // For other person messages (isMe=false): swipe left-to-right (positive dx)
    const panResponder = useRef(
        PanResponder.create({
            ...
            onPanResponderMove: (_: any, gs: any) => {
                ...
                if (shouldSwipe) {
                    const value = isMe ? Math.max(gs.dx, -(THRESHOLD + 20)) : Math.min(gs.dx, THRESHOLD + 20);
...
function MultiSelectBar({
    count, onCancel, onDeleteForMe, onDeleteForEveryone,
}: {
    count: number; onCancel: () => void;
...
function MultiSelectFooter({
    count, onDeleteForMe, onDeleteForEveryone,
}: {
...
export default function ChatScreen() {
    ...
    // FIX: map of message id → layout y-offset for scroll-to-reply
    ...
    useFocusEffect(
        useCallback(() => {
            ...
            const init = async () => {
                ...
                // FIX: Real-time listener — replaces one-time fetch
    ...
    // ─── Emoji react (FIX: one emoji per user — toggle same, replace different) 
    ...
    // ─── Long press on message (FIX: trigger on hold start, show on release) ─
    ...
    // FIX: scroll to original message when tapping reply preview
    ...
    const handleMenu = () => {
        ...
        options.push({
            label: "Select Messages", icon: "checkmark-circle-outline",
    ...
    const renderItem = ({ item }: { item: ListItem }) => {
        ...
        // FIX: check deletedForEveryone OR if current user is in deletedFor array
        const isDeletedForEveryone = item.deletedForEveryone === true;
        const isDeletedForMe = !isDeletedForEveryone &&
            Array.isArray(item.deletedFor) && item.deletedFor.includes(currentUserId);

        // If deleted for me only, don't render
        ...
        const messageContent = (
            ...
            >
                ...
                <View style={isMe ? styles.myBubbleCol : styles.theirBubbleCol}>
                    {/* FIX: tapping reply preview scrolls to original message */}
                    ...
                    <Pressable
                        ...
                        style={[styles.bubble, isMe ? styles.myBubble : styles.theirBubble, isDeletedForEveryone && styles.deletedBubble]}
                        delayLongPress={350}
                    >
                        {isDeletedForEveryone ? (
                            <Text style={[styles.bubbleText, styles.deletedText]}>
                                {/* FIX: show correct name for deleted message */}
                                ...
                                .map(([emoji, users]) => (
                                    <TouchableOpacity
                                        ...
                                        style={[
                                            ...
                                            (users as string[]).includes(currentUserId ?? "") && styles.reactionBadgeActive,
                                        ]}
                                        onPress={() => {
            ...
            {multiSelect ? (
                ...
                <View style={styles.header}>
                    ...
                    <View style={styles.headerContent}>
                        <AvatarWithFallback
                            uri={avatarUri} name={ownerName} size={40}
                            style={styles.headerAvatar} fallbackFontSize={18}
                        />
                    ...
                    <View style={styles.suggestedContainer}>
                        ...
                        <View style={styles.suggestedButtons}>
                            {SUGGESTED_MESSAGES.map((msg, i) => (
                                <TouchableOpacity
                                    key={i} style={styles.suggestedButton}
                                    onPress={() => sendMessageHandler(msg)} activeOpacity={0.7}
                                >
...
const styles = StyleSheet.create({
    ...
    messageWrap: { flexDirection: "row", marginBottom: 4, alignItems: "flex-end" },
    ...
    replyPreview: {
        ...
        paddingHorizontal: 10, paddingVertical: 6, marginBottom: 3, maxWidth: "100%",
    },
    ...
    inputBar: {
        ...
        backgroundColor: "white", borderTopWidth: 1, borderTopColor: "#eee", gap: 8,
    },
    input: {
    ...
    sendBtn: {
        backgroundColor: NAVY, width: 42, height: 42,

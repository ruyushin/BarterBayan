...
import { addComment, addCommentReply, deleteComment, getAllItems, getUserSavedItems, updateCommentLike, updateItemLikes, updateItemSave } from "../../services/itemService";
  ...
  return (
    <SafeAreaView style={styles.container}>
      <FlatList
        ...
        renderItem={({ item }) => <ItemCard item={item} onCommentAdded={() => setRefreshKey(prev => prev + 1)} />}
        ListHeaderComponent={
          <>
            {/* SEARCH */}
...
function ItemCard({ item, onCommentAdded }: any) {
  ...
  const currentUserPhotoURL = auth.currentUser?.photoURL || "https://i.pravatar.cc/150?img=1";
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setIsSaved(!isSaved);
      Alert.alert('Success', isSaved ? 'Item removed from saved' : 'Item saved successfully');
    } catch (error) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      setCommentsLoading(true);
      await sendMessage(currentUser, item.ownerId, `Hi, I'm interested in your ${item.title}`, item.id);
  ...
  const handleAddComment = async () => {
    if (!currentUser || !commentText.trim()) {
      if (!currentUser) Alert.alert('Please log in', 'You must be logged in to comment');
      return;
    }

    try {
      setCommentsLoading(true);
      const newComment = await addComment(item.id, currentUser, commentText.trim(), currentUserName, currentUserPhotoURL);
  ...
  const handleAddReply = async (commentId: string) => {
    if (!currentUser || !replyText.trim()) {
      if (!currentUser) Alert.alert('Please log in', 'You must be logged in to reply');
      return;
    }

    try {
      setCommentsLoading(true);
      await addCommentReply(item.id, commentId, currentUser, replyText.trim(), currentUserName, currentUserPhotoURL);
    ...
    try {
      ...
      const updatedComments = comments.map(c => {
        if (c.id === commentId) {
          ...
          return {
            ...c,
            likedBy: !commentLiked ? [...likedByArray, currentUser] : likedByArray.filter((id: string) => id !== currentUser),
            likes: !commentLiked ? (c.likes || 0) + 1 : Math.max((c.likes || 0) - 1, 0),
          };
        }
        return c;
      });
    ...
    <View style={styles.card}>
      ...
      <View style={styles.cardHeader}>
        <View style={styles.userInfo}>
          <Image 
            source={{ uri: item.userAvatar || "https://i.pravatar.cc/150?img=1" }} 
            style={styles.avatar} 
          />
        ...
        <View style={styles.statRow}>
          <View style={styles.stat}>
            <Pressable onPress={handleLike} disabled={loading}>
              <Ionicons name={isLiked ? "heart" : "heart-outline"} size={18} color={isLiked ? "#FF4444" : "#666"} />
            </Pressable>
            <Text style={styles.statText}>{likes}</Text>
          </View>

          <Pressable style={styles.stat} onPress={() => setShowComments(!showComments)}>
            <Ionicons name="chatbubble-outline" size={18} color="#666" />
            <Text style={styles.statText}>{comments.length}</Text>
          </Pressable>

          <Pressable style={styles.stat} onPress={handleSave} disabled={commentsLoading}>
            <FontAwesome name={isSaved ? "bookmark" : "bookmark-o"} size={16} color={isSaved ? "#5D5FEF" : "#666"} />
                ...
                <View style={styles.commentItem}>
                  ...
                  <View style={styles.commentContent}>
                    ...
                    <View style={styles.commentActions}>
                      ...
                      >
                        <Ionicons name={isCommentLiked ? "heart" : "heart-outline"} size={14} color={isCommentLiked ? "#FF4444" : "#999"} />
                        <Text style={styles.commentLikeText}>{comment.likes || 0}</Text>
                      </Pressable>
                      <Pressable 
                        onPress={() => setReplyingToCommentId(replyingToCommentId === comment.id ? null : comment.id)}

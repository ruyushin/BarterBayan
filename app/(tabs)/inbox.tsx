...
export default function InboxScreen() {
  ...
  const handleConversationPress = (conversation: any) => {
    router.push({
      pathname: "/chat",
      params: { ownerUserId: conversation.otherUserId },
    });
  };
  ...
  const renderMessage = ({ item }: any) => (
    <TouchableOpacity
      ...
      activeOpacity={0.75}
    >
      {/* Avatar — tap goes to profile, not chat */}
      <TouchableOpacity
        onPress={() => handleUserProfilePress(item.otherUserId)}
        activeOpacity={0.8}
      >
  ...
  const renderNotification = ({ item }: { item: Notification }) => {
    ...
    return (
      ...
      >
        ...
        {/* Icon / avatar — tap goes to the other user's profile when applicable */}
        <TouchableOpacity
          onPress={() =>
            item.otherUserId && !selectionMode
              ? handleUserProfilePress(item.otherUserId)
...
const styles = StyleSheet.create({
  ...
  // Messages
  messageRow: { flexDirection: "row", alignItems: "center", marginBottom: 18 },
  ...
  letterAvatarText: { color: "#fff", fontSize: 18, fontWeight: "700" },
  unreadBadge: {
    position: "absolute",
    top: 0,
    right: 0,

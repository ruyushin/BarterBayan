    ...
    <View style={ratingStyles.wrapper}>
      <View style={ratingStyles.starsRow}>
        {stars.map(({ filled, half, index }) => (
          <Text
            ...
            style={[
              ratingStyles.star,
              { fontSize: size, color: filled || half ? STAR_FILLED : STAR_EMPTY },
            ]}
          >
            {filled ? '★' : half ? '★' : '☆'}
          </Text>
      ...
      <View style={ratingStyles.ratingInfo}>
        ...
        {ratingCount !== undefined && ratingCount > 0 && (
          <Text style={ratingStyles.ratingCount}>({ratingCount} {ratingCount === 1 ? 'review' : 'reviews'})</Text>
        )}
      </View>
    </View>
  );
...
function SettingsRow({
  ...
  return (
    <Animated.View style={{ transform: [{ scale }] }}>
      ...
      >
        <View style={{ flex: 1 }}>
          <Text style={[styles.settingsRowLabel, danger && { color: ACCENT_RED }]}>{label}</Text>
          {subtitle ? <Text style={styles.settingsRowSubtitle}>{subtitle}</Text> : null}
        </View>
        <Text style={[styles.settingsRowChevron, danger && { color: ACCENT_RED }]}>›</Text>
      </TouchableOpacity>
    </Animated.View>
  );
}
            ...
            {submitted ? (
              ...
              <>
                ...
                <View style={reportStyles.photoRow}>
                  ...
                  {/* Add button (hidden once limit is reached) */}
                  {photos.length < 3 && (
                    <TouchableOpacity
...
export default function ProfileScreen() {
  ...
  const fetchProfile = async (currentUser: User) => {
    ...
    try {
      ...
      setError(isOffline ? 'You appear to be offline. Showing cached data.' : 'Failed to load profile.');
      setUserData({
        email: currentUser.email ?? undefined,
  ...
  if (error && !userData) {
    return (
      <View style={styles.errorContainer}>
        ...
        <TouchableOpacity style={styles.goLoginButton} onPress={() => router.replace('/login')}>
        ...
        <Animated.View style={{ opacity: fadeAnim, transform: [{ translateY: slideAnim }] }}>
          ...
          <View style={styles.avatarSection}>
            ...
            <TouchableOpacity
              ...
              onPress={() => router.push('/overview' as any)}
              activeOpacity={0.8}
            >
          ...
          <View style={styles.statsRow}>
            <StatCard emoji="🔄" label="Trades" count={userData?.tradesCount ?? 0} />
            <View style={styles.statDivider} />
            <StatCard emoji="📤" label="Exchanged" count={userData?.exchangedCount ?? 0} />
            <View style={styles.statDivider} />
            <StatCard emoji="🔖" label="Saved" count={userData?.savedCount ?? 0} />
          </View>
